...
try {
  ...
  function concatArrays(arr1, arr2) {
    return arr1.concat(arr2);// write me!
  }
  ...
  function returnAsNumbers(arr) { // return an array of nonNanny strings cast to Number
    let newArray = arr.filter(element => !(isNaNyString(element)));
    let newNumberArray = newArray.map(element => Number(element));
    return newNumberArray;// write me!
  ...
  function sumAllNumberys(arr) {
    let newArray = arr.filter(item => !(isNaN(item)));// write me!
    let numArray = newArray.map(item => Number(item))
    return sumAll(numArray);//numArray.forEach(x =>sumAll(x));// early return condition: array contains no numbery strings
  };
  ...
  function findAllEvens(arr) {
    ...
    return newArray.filter(item => ((Number(item)) % 2 === 0 ));// write me!
    // early return condition: array contains no numbery strings
  };
  ...
  function findAllOdds(arr) {
    let newArray = arr.filter(item => !(isNaN(item)));// write me!
    console.log(newArray);
    return newArray.filter(item => ((Number(item)) % 2 === 1 ));// write me!
    // early return condition: array contains no numbery strings
  };
